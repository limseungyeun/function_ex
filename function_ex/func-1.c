...
int main(void) { //시작
	...
	result = Add(a, b); //Add 함수 호출, 인수 있음(정수 a,b)
...
int Input(void) { //사용자 정의 함수, 매개 변수 없음, 정수 입력 부분
	int input;
	scanf("%d", &input); //정수형 숫자 입력
	return input; //값을 반환(보냄)
	//int input()->int 반환되는 값의 형 의미
}
